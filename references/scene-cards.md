...
- 圆形头像（源文件 `assets/avatar.png`，输出HTML同目录引用 `avatar.png`，120px，`border: 4px solid #F4D758`）
- 署名 Brclio 44px + 介绍「写代码，教编程，做产品」34px（以 `identity.md` 为准）
...
- 圆形头像 + 署名 Brclio（以 `identity.md` 为准）
- CTA：「关注 Brclio」（英文卡片使用「Stay curious. Keep building.」，以 `identity.md` 为准）
- 一行小字：固定签名档（统一读取 `identity.md`，代码示例见 `components.md` #43，中英文按卡片语言选择）
- 底部品牌三色装饰条
...
- 作者名、简介、头像、IP角色、签名与关注链接统一读取 `identity.md`；仅使用该文件列出的当前资料
- 头像源文件：`assets/avatar.png`（HTML内用相对路径 `avatar.png` 引用，交付时复制到输出HTML同目录）
- 署名固定为 Brclio，与 `identity.md` 及 `template-cards.html` 保持一致
- 品牌三色比例：主色6 : 强调3 : 点缀1
...
- 尾页 CTA 用「Stay curious. Keep building.」，签名档用英文版（统一读取 `identity.md`，代码示例见 `components.md` #43）
...
- [ ] 品牌三色比例 6:3:1
- [ ] 头像和署名是否正确（圆形头像+Brclio），简介、签名与当前链接均已按 `identity.md` 核对
