...
> ⚠️ **使用前请确认配置：** 下面是默认三色配色，可直接使用。如需换成你自己的品牌色，替换三个色值并同步修改模板 `:root` 变量。当前作者资料见 `references/identity.md`；头像与角色素材已配置为 Brclio。
...
Brclio 头像位于 `assets/avatar.png`（正方形，1254×1254px）。身份和账号以 `references/identity.md` 为准。

Brclio 的透明全身 IP 角色位于 `assets/character.png`。
